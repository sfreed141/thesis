...
A triangular patch (defined by three vertices) is subdivided into multiple subtriangles. There are four total tesselation levels: one which corresponds to the inner tesselation and three for the outer tesselation. Loosely speaking, the inner tesselation determines how many inner triangles are generated and the outer tesselation determines how many vertices are placed along each outer edge. See Figure~\ref{fig:tesselation} for an example.

\begin{figure}[h]
\centering
\includegraphics[scale=1.0]{openglwiki_tesselation.png}
\caption{A triangle tesselated with an inner tesselation level of 5 and outer tesselation levels of 4, 1, and 6. The dashed lines show the new triangles generated from the original triangle. (image from of OpenGL Wiki~\cite{openglwiki_tesselation}).}
